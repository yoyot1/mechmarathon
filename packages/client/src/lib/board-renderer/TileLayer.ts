...
import type { Board, CheckpointConfig, Tile, Direction } from '@mechmarathon/shared';
import {
  TILE_SIZE,
  TILE_GAP,
  TILE_COLORS,
...
export class TileLayer {
  ...
  build(board: Board, checkpoints: CheckpointConfig[]): void {
    ...
    for (let y = 0; y < board.height; y++) {
      for (let x = 0; x < board.width; x++) {
        ...
        this.drawWalls(tile, px, py);
        this.drawSymbol(tile, px, py);
      }
    }
  ...
  private drawSymbol(tile: Tile, px: number, py: number): void {
  ...
  private drawCheckpoint(num: number, px: number, py: number): void {
    // Badge background
