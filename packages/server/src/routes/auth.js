...
authRouter.post('/register', async (req, res) => {
  ...
  if (!parsed.success) {
    ...
    const error = { error: firstIssue.message, field: firstIssue.path[0] };
  ...
  if (existing) {
    ...
    const error = { error: `${field === 'email' ? 'Email' : 'Username'} already taken`, field };
  ...
  const response = { token, user: toUserProfile(user) };
