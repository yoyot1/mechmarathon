...
function rotateRobot(robot: Robot, rotation: 'cw' | 'ccw' | '180'): ExecutionEvent[] {
...
export function moveRobot(
  ...
  const direction: Direction = steps >= 0 ? robot.direction : oppositeDirection(robot.direction);
...
function killRobot(robot: Robot): void {
  robot.lives -= 1;
  robot.health = 0;
}
...
export function processExpressConveyors(robots: Robot[], board: Board): ExecutionEvent[] {
  return processConveyorType(robots, board, 'express_conveyor');
}

/** Process all conveyors (phase 2 of conveyor processing) */
export function processAllConveyors(robots: Robot[], board: Board): ExecutionEvent[] {
  return processConveyorType(robots, board, 'both');
}

function processConveyorType(
  robots: Robot[],
  board: Board,
  type: 'express_conveyor' | 'both',
): ExecutionEvent[] {
  const events: ExecutionEvent[] = [];
...
export function processGears(robots: Robot[], board: Board): ExecutionEvent[] {
  const events: ExecutionEvent[] = [];
...
export function processRepair(robots: Robot[], board: Board): ExecutionEvent[] {
  const events: ExecutionEvent[] = [];
...
export function checkWinCondition(robots: Robot[], totalCheckpoints: number): string | null {
  for (const robot of robots) {
